...
import { apiGetCategoryByIds, } from 'api@admin/category'
import {
...
const settingCategory = {
  key: 'module_tin_giao_phan_categories',
  value: [],
  serialize: true,
}
...
const defaultState = () => {
  return {
    module_tin_giao_phan_categories: settingCategory,
    moduleData: {
      code: 'module_tin_giao_phan',
      keys: [settingCategory],
    },
    infoCategory: {
      category_name: '',
      category_id: null,
...
export default {
  ...
  getters: {
    ...
    settingCategory(state) {
      return state.module_tin_giao_phan_categories
    },
    moduleData(state) {
  ...
  mutations: {
    ...
    [MODULE_UPDATE_RESET_SETTING_CATEGORY_VALUE_DATA](state, payload) {
      state.module_tin_giao_phan_categories.value = payload
    },

    [MODULE_UPDATE_SET_KEYS_DATA](state, payload) {
      state.module_tin_giao_phan_categories =
                payload.module_tin_giao_phan_categories
      state.moduleData.keys = []
      state.moduleData.keys.push(payload.module_tin_giao_phan_categories)
    },
      ...
      apiGetSettingByCode(
        ...
        (res) => {
          if (Object.keys(res.data.results).length) {
            ...
            dispatch(
              ACTION_GET_CATEGORY_LIST_BY_IDS,
              res.data.results.module_tin_giao_phan_categories
                .value
            )
          } else {
