...
const settingCategory = {
  key: 'module_loi_chua_categories',
  value: [],
  serialize: true,
}
...
export default {
  ...
  mutations: {
    ...
    [MODULE_UPDATE_RESET_SETTING_CATEGORY_VALUE_DATA](state, payload) {
      state.module_loi_chua_categories.value = payload
    },

    [MODULE_UPDATE_SET_KEYS_DATA](state, payload) {
      state.module_loi_chua_categories =
                payload.module_loi_chua_categories
      ...
      apiGetSettingByCode(
        ...
        (res) => {
          if (Object.keys(res.data.results).length) {
            ...
            dispatch(
              ACTION_GET_CATEGORY_LIST_BY_IDS,
              res.data.results.module_loi_chua_categories.value
            )
          } else {
