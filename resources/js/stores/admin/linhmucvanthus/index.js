  ...
  actions: {
    ...
    async [ACTION_DELETE_INFO_BY_ID]({ state, dispatch, commit, }, infoId) {
      ...
      if (typeof state.infoDelete === 'object') {
        if (fnCheckProp(state.infoDelete, 'id')) {
    ...
    [MODULE_MODULE_VAN_THU + '_' + ACTION_RELOAD_GET_INFO_LIST]: {
      root: true,
      handler(namespacedContext, payload) {
        if (isNaN(payload)) {
          return fn_redirect_url(`/${config.adminPrefix}/van-thus`)
        } else {
