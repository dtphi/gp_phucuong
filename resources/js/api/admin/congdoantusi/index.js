...
export const apiGetInfoById = (infoId, resolve, errResole) => {
  return axios.get(fn_get_base_api_detail_url(API_CONG_DOAN_TU_SIS_RESOURCE, infoId))
    .then((response) => {
...
export const apiGetInfos = (resolve, errResole, params) => {
  return axios.get(fn_get_base_api_url(API_CONG_DOAN_TU_SIS_RESOURCE), {
    params: params,
  })
    .then((response) => {
      if (response.status === 200) {
...
export const apiUpdateInfo = (info, resolve, errResole) => {
  return axios.put(fn_get_base_api_detail_url(API_CONG_DOAN_TU_SIS_RESOURCE, info.id), info)
    .then((response) => {
      if (response.status === 200) {
        var json = {}
...
export const apiInsertInfo = (info, resolve, errResole) => {
  return axios.post(fn_get_base_api_url(API_CONG_DOAN_TU_SIS_RESOURCE), info)
    .then((response) => {
...
export const apiDeleteInfo = (infoId, resolve, errResole) => {
  return axios.delete(fn_get_base_api_detail_url(API_CONG_DOAN_TU_SIS_RESOURCE, infoId))
    .then((response) => {
      if (response.status === 200) {
        var json = {}
        json['data'] = response.data
