...
export const apiGetInfoById = (infoId, resolve, errResole) => {
  return axios.get(fn_get_base_api_detail_url(API_CO_SOS_RESOURCE, infoId))
    .then((response) => {
...
export const apiUpdateInfo = (info, resolve, errResole) => {
  return axios.put(fn_get_base_api_detail_url(API_CO_SOS_RESOURCE, info.id), info)
    .then((response) => {
      console.log(response)
      if (response.status === 200) {
        var json = {};
...
export const apiInsertInfo = (info, resolve, errResole) => {
  return axios.post(fn_get_base_api_url(API_CO_SOS_RESOURCE), info)
    .then((response) => {
      console.log(response)
...
export const apiDeleteInfo = (infoId, resolve, errResole) => {
  return axios.delete(fn_get_base_api_detail_url(API_CO_SOS_RESOURCE, infoId))
    .then((response) => {
      console.log(response)
      if (response.status === 200) {
        var json = {};
