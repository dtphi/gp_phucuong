...
export const apiGetInfoById = (infoId, resolve, errResole) => {
  return axios.get(fn_get_base_api_detail_url(API_CHUCVUS_RESOURCE, infoId))
    .then((response) => {
...
export const apiGetInfos = (resolve, errResole, params) => {
  return axios.get(fn_get_base_api_url(API_CHUCVUS_RESOURCE), {
    params: params,
  })
    .then((response) => {
      if (response.status === 200) {
...
export const apiInsertInfo = (info, resolve, errResole) => {
  return axios.post(fn_get_base_api_url(API_CHUCVUS_RESOURCE), info)
    .then((response) => {
...
export const apiDeleteInfo = (infoId, resolve, errResole) => {
  return axios.delete(fn_get_base_api_detail_url(API_CHUCVUS_RESOURCE, infoId))
    .then((response) => {
      if (response.status === 200) {
        var json = {}
        json['data'] = response.data
