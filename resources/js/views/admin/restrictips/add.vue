      ...
      <div class="container-fluid">
        <div class="panel panel-default">
          ...
          <div class="panel-body">
            <info-add-form ref="formAddRestrictIp"></info-add-form>
          </div>
        </div>
      </div>
    </validation-observer>
...
export default {
  ...
  methods: {
    ...mapActions(MODULE_MODULE_RESTRICT_IP_ADD, [ACTION_RESET_NOTIFICATION_INFO]),
    _errorToArrs() {
      let errs = [];
      if (
        this.errors.length &&
    ...
    _submitInfo() {
      ...
      _self.$refs.observerInfo.validate().then((isValid) => {
        if (isValid) {
          _self.$refs.formAddRestrictIp._submitInfo();
        }
      });
    },
    _submitInfoBack() {
      ...
      _self.$refs.observerInfo.validate().then((isValid) => {
        if (isValid) {
          _self.$refs.formAddRestrictIp._submitInfoBack();
        }
      });
    },
