...
const pseudo_markdown = {
    ...
    [type_markdown.multiline]: {
        end: [ type_markdown.multiline ],
        allowed_chars: '(.|\n)',
        type: 'multiline-code'
    },
    [type_markdown.inline]: {
        end: [ type_markdown.inline ],
        allowed_chars: '.',
        type: 'inline-code'
    }
    // ':': {
...
function compileToJSON(str) {
    ...
    if (min_index_of_key) {
        ...
        const match = str_right.match(
            new RegExp('^(' + (pseudo_markdown[char].allowed_chars || '.') + '*' + (pseudo_markdown[char].end ? '?' : '') + ')' + (pseudo_markdown[char].end ? '(' + pseudo_markdown[char].end + ')' : ''), 'm')
        );
...
function markdownResult(array) {
    for (let i = 0; i < array.length; i) {
        if (array[i - 1]) {
            const isInline = array[i].types.indexOf('inline-code') !== -1 && array[i - 1].types.indexOf('inline-code') !== -1;

            const isMultiline = array[i].types.indexOf('multiline-code') !== -1 && array[i - 1].types.indexOf('multiline-code') !== -1;
