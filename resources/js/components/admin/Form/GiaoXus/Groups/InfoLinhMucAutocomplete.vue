...
export default {
  ...
  computed: {
    ...mapState(MODULE_MODULE_LINH_MUC, {
      dropdowns: (state) => state.dropdownLinhMucs,
    }),
    _linhmucs() {
      if (this.searchName) {
  ...
  methods: {
    ...mapActions(MODULE_MODULE_LINH_MUC, ['ACTION_GET_DROPDOWN_LINH_MUC_LIST']),
    _searchCategories() {
      const query = this.query
      if (query && query.length) {
        this.ACTION_GET_DROPDOWN_LINH_MUC_LIST(query)
      }
    },
    _focusParentCategory() {
      if (this.dropdowns.length == 0) {
        this.ACTION_GET_DROPDOWN_LINH_MUC_LIST('')
      }
    },
    _closeDropdown() {
    ...
    _addInfoToCategory(infoCategory) {
      this.$emit('on-select-linh-muc', infoCategory)
      this._closeDropdown()
    },
    _keyUpName(evt) {
