...
export default {
  name: 'CongDoanNgoaiAutocomplete',
  props: {
    name: {
      default: null,
    },
  ...
  computed: {
    ...mapState(MODULE_MODULE_LINH_MUC, {
      dropdowns: (state) => state.dropdownCongDoanNgoaiGiaoPhans,
    }),
    _cosos() {
      if (this.searchName) {
  ...
  methods: {
    ...mapActions(MODULE_MODULE_LINH_MUC, [
      'ACTION_GET_DROPDOWN_CONG_DOAN_NGOAI_GIAO_PHAN_LIST'
    ]),
    _searchCategories() {
      const query = this.query; console.log(query)
      if (query && query.length) {
        this.ACTION_GET_DROPDOWN_CONG_DOAN_NGOAI_GIAO_PHAN_LIST(query)
      }
    },
    _focusParentCategory() {
      if (this.dropdowns.length == 0) {
        this.ACTION_GET_DROPDOWN_CONG_DOAN_NGOAI_GIAO_PHAN_LIST('')
      }
    },
    _closeDropdown() {
    ...
    _addInfoToCategory(infoCategory) {
      this.$emit('on-select-cong-doan-ngoai-giao-phan', infoCategory)
      this._closeDropdown()
    },
    _keyUpName(evt) {
