...
class SettingController extends Controller
{
     ...
     *
     * @api POST /v1/representative/setting/change-password
     */
    public function changePassword(Request $request)
    {
        $representative = Representative::find($this->guard()->id());
        ...
        if (!Hash::check($request->old_password, $representative->password)) {
     ...
     * @return \Illuminate\Http\JsonResponse
     *
     * @api POST /v1/representative/setting/change-email
     */
    public function changeEmail(Request $request)
    {
        ...
        $rules = [
            'main_email' => 'required|email|max:255|different:sub_email1|different:sub_email2|different:sub_email3|different:sub_email4',
        ...
        if (Store::checkMainEmailExists($request->main_email) || Representative::checkMainEmailExists($request->main_email, $representative->id)) {
            return Helper::errorResponse([ 'main_email' => [ 'このメインメールアドレスはすでにシステムに登録されています。' ] ], Response::HTTP_UNPROCESSABLE_ENTITY);
        }

        $representative->update($request->only('main_email', 'sub_email1', 'sub_email2', 'sub_email3', 'sub_email4'));
