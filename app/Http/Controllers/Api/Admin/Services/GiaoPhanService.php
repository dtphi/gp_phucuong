...
final class GiaoPhanService implements BaseModel, GiaoPhanModel
{
  ...
  public function apiGetList(array $options = [], $limit = 15)
  {
    ...
    // get all list giao_phans with perPage = -1
    if ($limit == -1) {
      $results = $query->get();
    ...
    if ($this->model->save()) {
        ...
        foreach ($data['hats'] as $hat) {
          ...
          if (isset($hat['giao_xus'])) {
            foreach ($hat['giao_xus'] as $giaoXu) {
              GiaoPhanHatXu::insertByGiaoHatId(
                $giaoPhanId,
                $giaoPhanHat->id,
                $hat['giao_hat_id'],
                $giaoXu['giao_xu_id'],
          ...
          if (isset($hat['cong_doan_tu_sis'])) {
            foreach ($hat['cong_doan_tu_sis'] as $congDts) {
              GiaoPhanHatCongDoanTuSi::insertByGiaoHatId(
                $giaoPhanId,
                $giaoPhanHat->id,
                $hat['giao_hat_id'],
                $congDts['cong_doan_tu_si_id'],
                $congDts['active']
              );
            }
          }
      ...
      if (isset($data['banchuyentrachs']) && !empty($data['banchuyentrachs'])) {
        foreach ($data['banchuyentrachs'] as $banCt) {
          GiaoPhanBanChuyenTrach::insertByGiaoPhanId(
            $giaoPhanId,
            $banCt['ban_chuyen_trach_id'],
            $banCt['active']
          );
        }
      }
      ...
      if (isset($data['hats']) && !empty($data['hats'])) {
        ...
        foreach ($giaoDiems as $gdiems) {
          foreach ($gdiems as $gdiem) {
            GiaoPhanHatXuDiem::insertByGiaoPhanId(
              $giaoPhanId,
              $gdiem->giao_hat_id,
              $gdiem->giao_xu_id,
              $gdiem->giao_diem_id,
